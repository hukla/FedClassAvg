...
global_model, global_classifier = global_model.to(device), global_classifier.to(device)
client_models = []
client_classifiers = []
...
val_dataloader = DataLoader(val_dataset, batch_size=configs['batch_size'], shuffle=False, num_workers=4, pin_memory=True)
test_dataloader = DataLoader(test_dataset, batch_size=configs['batch_size'], shuffle=False, num_workers=4, pin_memory=True)
...
if mpi_rank == 0:
    if 'sweep' not in output_path:
        torch.save(global_classifier, os.path.join(output_path, f'round0_global_classifier.pt'))
...
for round_idx in range(1, args.max_rounds + 1):
    ...
    sum_state_dict = utils.zero_state_dict_like(global_state_dict) # to be local sum
    ...
    for active_idx, active in enumerate(virtual_nodes):
        ...
        val_loss, val_acc = heteropfl_utils.evaluate_cl(model, classifier, client_test_loader, device)
    ...
    if args.tsne and mpi_rank == 0:
        ...
        preds, label_arr, client_label, performance = utils.generate_tsne_features_cl(client_models[:len(virtual_nodes)], client_classifiers, test_dl, device)
    ...
    for active_idx, active in enumerate(virtual_nodes):
        ...
        val_loss, val_acc = heteropfl_utils.evaluate_cl(model, classifier, test_dataloader, device)
        round_test_loss += val_loss
        round_test_acc += val_acc
