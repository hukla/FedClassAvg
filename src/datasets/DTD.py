...
class DTDDataset(torch.utils.data.Dataset):
    def __init__(self, label_path, transforms=None,image_dir=None, if_train=True):
        if not image_dir:
        ...
        if transforms:
            ...
            self.transforms = DTDTransform(224, if_train=if_train)
        ...
        self.configs={'lr': 0.002, 'batch_size': 32}
...
def compute_image_mean(label_path):
    image_size = 224
    dataset = DTDDataset(label_path, 
...
if __name__ == "__main__":
    image_size = 224
    dataset = DTDDataset('/data/dtd/labels/train1.txt')
    ...
    for images, labels in image_loader:
        ...
        for idx, (image, label) in enumerate(zip(images, labels)):
            ...
            axes[idx].set_title(f'label:{label}')
    # for i in range(1, 11):
        # compute_image_mean(f'/data/dtd/labels/train{i}.txt')
