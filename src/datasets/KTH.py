    ...
    def __init__(self, image_size, if_train=True):
        if if_train:
            self.data_transform = transforms.Compose([
                    ...
                    transforms.ToTensor(),
                    transforms.Normalize((0.5238,0.4476,0.3119), (0.1739,0.1439,0.1586)),
                ])
        else:
            self.data_transform = transforms.Compose([
                    transforms.Resize((image_size, image_size)),
...
class KTHDataset(torch.utils.data.Dataset):
    def __init__(self, label_path, if_train=True, transforms=None,image_dir=None):
        ...
        if transforms:
            ...
            self.transforms = KTHTransform(224, if_train=if_train)
        ...
        self.configs={'lr': 0.0001, 'batch_size': 32}
...
if __name__ == "__main__":
    ...
    for i in range(1, 11):
        dataset = KTHDataset(f'/fl_workspace/data/KTH/labels/train{i}.txt',
